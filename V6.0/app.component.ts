...
let api: string;
const DATA2 = "http://localhost:3000/";
...
export class AppComponent implements OnInit {
    ...
    ngOnInit() {
        this.appservice.getJSON(DATA2 + "db").subscribe(res =>
            this.bbdd = Object.keys(res),
            error => this.errorMessage = error,
            () => {
                api = this.bbdd[0];
                this.appservice.getJSON(DATA2 + api + "/").subscribe(res =>
                    this.datos = res,
                    error => this.errorMessage = error,
                    () => {
                        this.sacar();
    ...
    addb(test) {
        ...
        if (this.validar(test)) {
            this.appservice.add(DATA2 + api + "/", this.test).subscribe(
                data => null,
    ...
    modificar(post) {
        if (this.validar(post)) {
            this.appservice.update(DATA2 + api + "/", post).subscribe(
                data => null,
                error => this.errorMessage = <any>error,
                () => this.appservice.getJSON(DATA2 + api + "/").subscribe(res =>
                    this.datos = res)
    ...
    delet(post) {
        this.appservice.delete(DATA2 + api + "/", post.id).subscribe(
            data => null,
            error => this.errorMessage = <any>error,
            () => this.appservice.getJSON(DATA2 + api + "/").subscribe(res =>
                this.datos = res)
